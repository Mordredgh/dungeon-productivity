...
const restore = read('scripts/supabase-restore-test.ps1');
const freeDrill = read('scripts/supabase-free-tier-drill.ps1');
const localDrill = read('scripts/supabase-free-tier-local-restore-drill.ps1');
...
assert.match(restore, /xibmopqlgjbcypxixnri\|stdedxhxxoyostymldqn/);
assert.match(restore, /DockerNetwork/);
assert.match(freeDrill, /supabase-backup\.ps1/);
assert.match(freeDrill, /supabase-restore-test\.ps1/);
assert.match(localDrill, /beta_restore_probe/);
assert.match(localDrill, /Drill local Free Tier completo/);
...
assert.match(runbook, /supabase-free-tier-drill\.ps1/);
